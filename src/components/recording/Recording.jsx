...
const Recording = () => {
    ...
    const handleInput = (e) => {
        ...
        axios.post('http://localhost:3000/api/recording', data)
        .then((res)=>{
            setData(res.data)
        })
    }
  return (
    <div style={{ background: '#081b29' }} className='w-screen min-h-screen overflow-auto py-4 justify-center items-center flex flex-col gap-4'>
        ...
        >Lets do revison with our top mentors</motion.div>
        ...
        <div className='w-screen h-16 flex justify-center items-center gap-4 px-4 mt-4' >
            ...
            transition={{ duration: 1, }}
            type="text" placeholder='Search for a course' className='w-1/3 h-10 rounded-md border-2 border-gray-300 p-2'
            ...
            animate={{ opacity: 1, y: 0 }}  
            transition={{ duration: 1}}
            type="text" placeholder='Search for a topic' className='w-1/3 h-10 rounded-md border-2 border-gray-300 p-2'
            value={topic} onChange={(e)=>setTopic(e.target.value)} />
            <motion.input 
            initial={{ opacity: 0, x: 100 }}
            animate={{ opacity: 1, x: 0 }}  
            transition={{ duration: 1}}
            type="text" placeholder='Any expert in mind?' className='w-1/3 h-10 rounded-md border-2 border-gray-300 p-2'
  ...
  {data.map((i, index) => {
    ...
    return (
      ...
      >
        ...
        <h2 className="text-lg font-bold text-white mb-1">{i.title}</h2>

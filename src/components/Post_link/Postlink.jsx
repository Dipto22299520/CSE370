...
const Postlink = () => {
  ...
  // UI toggle states
  const [courseShown, setCourseShown] = useState(false);
  ...
  // Fetch user's available course/section data
  const getCourseinfo = async () => {
    try {
      const response = await axios.post('http://localhost:3000/api/postlink', { id, email });
  ...
  const handleLinkSubmit = async () => {
    try {
      const res = await axios.post('http://localhost:3000/api/meetlink', {
        id, name, course, section, link, description, classcontext
      });
      if (res.status === 200) {
  ...
  return (
    <div className="relative w-full min-h-screen grid grid-cols-1 md:grid-cols-2 justify-center items-center px-10 py-10 gap-10">
      ...
      <div className="grid grid-cols-1 gap-10">
        ...
        {/* Description Input */}
        <ToggleSection title="Who should join the class" toggle={joinShown} setToggle={setJoinShown}>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            className="w-full h-24 p-2 bg-slate-400/20 font-bold rounded-md"
            placeholder="Enter who should join..."
          />
        </ToggleSection>

        {/* Class Context Input */}
        <ToggleSection title="Context about the class" toggle={contextShown} setToggle={setContextShown}>
          <textarea
            value={classcontext}
            onChange={(e) => setClasscontext(e.target.value)}
            className="w-full h-24 p-2 bg-slate-400/20 font-bold rounded-md"
            placeholder="Enter class context..."
          />
        </ToggleSection>
      </div>

      {/* Right Panel (Link Input) */}
      {course && section && description && classcontext && (
